...
func (r ApiListQueryLambdasWithAliasRequest) Execute() (ListQueryLambdasResponse, *_nethttp.Response, error) {
	return r.ApiService.ListQueryLambdasWithAliasExecute(r)
}
 ...
 * @return ListQueryLambdasResponse
 */
func (a *AliasesApiService) ListQueryLambdasWithAliasExecute(r ApiListQueryLambdasWithAliasRequest) (ListQueryLambdasResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodGet
		localVarPostBody     interface{}
		localVarFormFileName string
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AliasesApiService.ListQueryLambdasWithAlias")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/orgs/self/ws/{workspace}/aliases/{alias}/lambdas"
	localVarPath = strings.Replace(localVarPath, "{"+"workspace"+"}", _neturl.PathEscape(parameterToString(r.workspace, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"alias"+"}", _neturl.PathEscape(parameterToString(r.alias, "")), -1)
